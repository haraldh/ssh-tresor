...
ssh-tresor derives encryption keys by asking the SSH agent to sign a challenge,
then uses the signature as key material for AES-256-GCM encryption. Secrets can
only be decrypted when the corresponding SSH key is loaded in an agent.
...
Examples:
  ...
  # Use in config files
  password_command = \"ssh-tresor decrypt ~/.config/app/secret.tresor\"";
